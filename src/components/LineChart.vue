  ...
  methods: {
    onChange(event) {
      ...
      if (event.target.value == "clickThruRate") {
        let adults = this.lists.records.map((person) => {
          return [person.date, person.clickThruRate];
        });
        this.$store.dispatch("setchartdata", hello.concat(adults));
      } else if (event.target.value == "pageViews") {
        let adults = this.lists.records.map((person) => {
          return [person.date, person.pageViews];
        });
        this.$store.dispatch("setchartdata", hello.concat(adults));
      } else if (event.target.value == "orders") {
        console.log("clickThruRate");
        let adults = this.lists.records.map((person) => {
          return [person.date, person.orders];
        });
        this.$store.dispatch("setchartdata", hello.concat(adults));
      } else {
        let adults = this.lists.records.map((person) => {
          return [person.date, person.sales];
        });
        this.$store.dispatch("setchartdata", hello.concat(adults));
      }
    },
  ...
  data() {
    return {
      ...
      chartOptions: {
        chart: {
          title: "Reports",
          subtitle: "Sales, pageViews, and orders",
        },
      },
    };
  },
